...
| `strategy-test.concurrency` | Параллельных проверок (по умолчанию 4) |
...
`strategy-test` используется только при подборе стратегии ByeDPI Auto. Его адреса обязательны, не наследуются и не заменяют `connectivity-check`. Кэш хранит только результат подбора стратегии. Старая секция `test` запрещена; её нужно переименовать в `strategy-test`.
