...
`refresh` هنگام اعمال پروفایل و هنگام اجرای نود بررسی می‌شود — تایمر دائمی جداگانه‌ای وجود ندارد.
...
`startup.max-age` (پیش‌فرض `30d`) سن مجاز کش قابل استفاده را محدود می‌کند.

کش کاری به فهرست نهایی resolverها، `domains` و نسخهٔ StormDNS گره خورده است. تغییر هرکدام کش تازه‌ای می‌سازد و کش قدیمی تنها پس از اعمال موفق پروفایل حذف می‌شود. اگر کش مناسبی نباشد یا StormDNS آن را نپذیرد، خودش به پویش کامل برمی‌گردد — این رفتار عادی است.
