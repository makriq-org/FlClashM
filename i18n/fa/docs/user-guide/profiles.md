...
StormDNS ترافیک TCP را در پرس‌وجوهای معمولی DNS به یک رزالور مجاز می‌پیچد — تا اتصال از لیست‌های سفید بگذرد. همان هدف OlcRTC، اما با حاملی متفاوت: جایی که تنها DNS عبور داده می‌شود. این نود **به‌طور محسوسی کندتر** از بقیه است.

پشتیبان خواب‌آلود دقیقاً مانند OlcRTC پیکربندی می‌شود — به بخش «فعال‌سازی (ذخیرهٔ خواب‌آلود)» در بالا نگاه کنید.
