...
sudo groupadd --system flclashm 2>/dev/null || true
helper_group=$(getent group flclashm | cut -d: -f3)
...
sudo unshare --mount --net --fork -- env \
  ...
  HELPER_GROUP="$helper_group" bash -ceu '
  mount --make-rprivate /
  mount -t tmpfs tmpfs /run
  mkdir -p /run/flclashm
  "$HELPER" --socket /run/flclashm/helper.sock &
  daemon=$!
  trap "kill $daemon 2>/dev/null || true" EXIT
  for _ in $(seq 1 30); do test -S /run/flclashm/helper.sock && break; sleep 0.1; done
  test -S /run/flclashm/helper.sock
  request() {
    printf "%s\\n" "$1" | setpriv --reuid="$CLIENT_UID" --regid="$CLIENT_GID" --groups="$HELPER_GROUP" "$HELPER" --request
  }
