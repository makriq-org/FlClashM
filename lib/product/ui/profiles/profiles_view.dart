...
typedef AddProfileViewBuilder = Widget Function(BuildContext context);
typedef EditProfileViewBuilder = Widget Function(
  BuildContext context,
  Profile profile,
);
typedef OverrideProfileViewBuilder = Widget Function(String profileId);
typedef ScriptsViewBuilder = Widget Function(BuildContext context);
typedef SendToTvPageBuilder = Widget Function(String profileUrl);

class ProfilesView extends StatefulWidget {
  const ProfilesView({
    super.key,
    required this.addProfileViewBuilder,
    required this.editProfileViewBuilder,
    required this.overrideProfileViewBuilder,
    required this.scriptsViewBuilder,
    required this.sendToTvPageBuilder,
  });

  final AddProfileViewBuilder addProfileViewBuilder;
  final EditProfileViewBuilder editProfileViewBuilder;
  final OverrideProfileViewBuilder overrideProfileViewBuilder;
  final ScriptsViewBuilder scriptsViewBuilder;
  final SendToTvPageBuilder sendToTvPageBuilder;
...
class _ProfilesViewState extends State<ProfilesView> with PageMixin {
  ...
  void _handleShowAddExtendPage() {
    final baseContext = globalState.navigatorKey.currentState!.context;
    showExtend(
      baseContext,
      builder: (_, type) => AdaptiveSheetScaffold(
        type: type,
        body: widget.addProfileViewBuilder(baseContext),
  ...
  List<Widget> get actions => [
        ...
        IconButton(
          ...
          onPressed: () {
            showExtend(
              context,
              builder: (_, __) => widget.scriptsViewBuilder(context),
            );
          },
          icon: Consumer(
                  ...
                  for (int i = 0;
                    ...
                    GridItem(
                      child: ProfileItem(
                        ...
                        editProfileViewBuilder: widget.editProfileViewBuilder,
                        overrideProfileViewBuilder:
                            widget.overrideProfileViewBuilder,
                        sendToTvPageBuilder: widget.sendToTvPageBuilder,
                      ),
                    ),
                ],
              ),
...
class _ProfileItemState extends State<ProfileItem> {
  ...
  void _handleShowEditExtendPage(BuildContext context) {
    showExtend(
      ...
      builder: (_, type) => AdaptiveSheetScaffold(
        ...
        body: widget.editProfileViewBuilder(context, widget.profile),
  ...
  void _handlePushGenProfilePage(BuildContext context, String id) {
    BaseNavigator.modal(
      context,
      widget.overrideProfileViewBuilder(id),
    );
  }
...
class _ReorderableProfilesSheetState extends State<ReorderableProfilesSheet> {
  ...
  void initState() {
    super.initState();
    profiles = List<Profile>.from(widget.profiles);
  }
