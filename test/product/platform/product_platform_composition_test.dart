...
void main() {
  ...
  group('ProductPlatformComposition', () {
    ...
    test(
      'uses desktop runtime and updater while TUN remains unavailable',
      () async {
        final composition = ProductPlatformComposition.forProfile(
        ...
        expect(
          (updater as AppUpdateService).platform,
        ...
        expect(
          globalState.runtimeRegistry.resolveSelection().engine.adapter,
          isNotNull,
        );
        await expectLater(
  ...
  test(
    'uses the stable desktop ID and runtime layout in every build target',
    () async {
      ...
      for (final artifact in ProductInstallLayout.artifacts.where(
        (artifact) => artifact != ProductInstallLayout.helperArtifact,
      )) {
      ...
      for (final artifact in ProductInstallLayout.artifacts.where(
        ...
        expect(windows, contains('\${RUNTIME_ASSET_DIR}/$artifact.exe"'));
      }
