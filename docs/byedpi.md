...
Автоподбор не перебирает весь список на старте. Клиент быстро проверяет
первые стратегии с коротким пределом ответа и, если подходящая стратегия не
найдена, запускает встроенную запасную стратегию
`--disorder 1 --auto=torst --tlsrec 1+s`. Это нужно, чтобы локальный SOCKS5
узел не оставался закрытым из-за долгого перебора. Выбранная стратегия
кэшируется для cold-start.
